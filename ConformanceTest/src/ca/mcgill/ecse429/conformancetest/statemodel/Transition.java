...
public class Transition {
	...
	public Transition(String aEvent, String aCondition, String aAction,
			State aFrom, State aTo) {
		event = aEvent;
		condition = aCondition;
		action = aAction;
		if (!setFrom(aFrom)) {
			throw new RuntimeException(
					"Unable to create Transition due to aFrom");
		}
		if (!setTo(aTo)) {
	...
	public String toString() {
		...
		return super.toString()
				...
				+ "  "
				+ "from = "
				+ (getFrom() != null ? Integer.toHexString(System
						.identityHashCode(getFrom())) : "null")
				+ System.getProperties().getProperty("line.separator")
				+ "  "
				+ "to = "
				+ (getTo() != null ? Integer.toHexString(System
						.identityHashCode(getTo())) : "null") + outputString;
	}
}